...
def mask_scaled_dot_product_attention(q, k, v, depth):
    a = torch.matmul(q, torch.t(k))
    a = a / math.sqrt(depth)
    a = F.softmax(a, dim=1)
    a = masking(a)
...
class Mask_MultiHeadAttention(N.Module):
    ...
    def forward(self, query, key, value):
        ...
        for q, k, v in zip(query_head, key_head, value_head):
            attention.append(mask_scaled_dot_product_attention(q, k, v, self.depth))
...
def main():
    ...
    target = torch.rand((7, 5))
    attention = Mask_MultiHeadAttention(5, 6, 7, 2)
    attention(source, source, source)
